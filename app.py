...
from im2recipe.src.models_new import get_model as get_model2
...
def img_ingr2recipe(im_path, ingr):
    ...
    def output_image(outs):
        print(outs[2])
        # outs[2] ex. 76b97d34f8.jpg
        path = "./inversecooking/data/demo_imgs/1.jpg"

        return Image.open(path)

    outs_all = infer(im_path, ' '.join(ingr))
    return_recipe1 = output_recipe(outs_all[0])
    return_image1 = output_image(outs_all[0])
    return_recipe2 = output_recipe(outs_all[1])
    return_image2 = output_image(outs_all[1])
    return_recipe3 = output_recipe(outs_all[2])
    return_image3 = output_image(outs_all[2])
    
    return return_image1, return_recipe1, return_image2, return_recipe2, return_image3, return_recipe3
...
def infer(im_path, sim_ingrs):
    ...
    # find top 3 recipes
    topk = find_top_k(recipefeats, embedding, ids, 3)
    # print(topk)
    titles, instructions = [], []
    output = []
    for _id in topk:
    ...
    with gr.Tabs():
        with gr.TabItem("User"):
            ...
            with gr.Row():
                ...
                out_image1 = gr.Image(label="Looks yummy ><", shape=(224, 200))
            with gr.Row():
